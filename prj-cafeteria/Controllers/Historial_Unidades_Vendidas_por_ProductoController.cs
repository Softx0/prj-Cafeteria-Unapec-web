...
namespace prj_cafeteria.Controllers
{
    public class Historial_Unidades_Vendidas_por_ProductoController : Controller
    {
        ...
        {
            string filename = "Reporte historial.csv";
            string filepath = @"c:\TEMP\" + filename;
            StreamWriter sw = new StreamWriter(filepath);
            sw.WriteLine("Total Vendido,Descripcion"); //Encabezado 
            foreach (var i in db.Historial_Unidades_Vendidas_por_Producto.ToList())
            {
                sw.WriteLine(i.Total.ToString() + "," + i.DESCRIPCION);
            }
            sw.Close();
